...
_croc_recv_cmd() {
    ...
    done <<< $(sqlite3 $ZDOTDIR/.confidential/croc_collaborators_registry.db "SELECT Name,ID,Designation FROM collaborator_catalogue WHERE ID!='$CROC_SELF_TRANSFER_ID' AND (Dormant!=1 OR Dormant IS NULL)")
